...
protected:
    ...
    //
    // The input minibatch to this layer. Computed by the previous layer - Has shape in_shape.
    //
    float *in_batch = nullptr;

    //
    // The output minibatch from this layer. Pointer shared with in_batch of
    // the next layer - Has shape out_shape.
    //
    float *out_batch = nullptr;


    //
    // Derivative wrt out_batch. Computed by the next layer - Has shape out_shape.
    //
    float *grad_out_batch = nullptr;

    //
    // Derivative wrt the input batch - Has shape in_shape.
    //

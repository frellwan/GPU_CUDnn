...
//
// If there is a previous layer, initialize this layer's input with the previous
// layer's' output (as well as the tensor descriptor).
//
Layer::Layer(std::shared_ptr<Layer> _prev, cublasHandle_t _cublasHandle, cudnnHandle_t _cudnnHandle) :
    prev(_prev),
...
{
        //set output tensor descriptor out_shape to have format
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    // Get the input tensor descriptor in_shape into the variables declared above
    CUDNN_CALL(cudnnGetTensor4dDescriptor(in_shape, &dtype, &n, &c, &h, &w,
        &n_stride, &c_stride, &h_stride, &w_stride));
    ...
    // Create & set a filter descriptor for a float array ordered
    ...
    // Create and set a convolution descriptor. This will
    ...
    CUDNN_CALL(cudnnGetConvolutionForwardAlgorithm_v7(cudnnHandle,
        in_shape, filter_desc, conv_desc, out_shape, 1, 0, &fwd_algo));
    CUDNN_CALL(cudnnGetConvolutionBackwardFilterAlgorithm_v7(cudnnHandle,
        in_shape, out_shape, conv_desc, filter_desc, 1, 0, &bwd_filter_algo));
    CUDNN_CALL(cudnnGetConvolutionBackwardDataAlgorithm_v7(cudnnHandle,
        filter_desc, out_shape, conv_desc, in_shape, 1, 0, &bwd_data_algo));
...
//
// Determines the largest workspace size needed by any of the convolution
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    // Create and set pooling descriptor to have the given mode,
...
: Loss(prev, cublasHandle, cudnnHandle)
{
    ...
    // Get descriptor of input minibatch, in_shape, into variables
    // declared above
    CUDNN_CALL(cudnnGetTensor4dDescriptor(in_shape, &dtype, &n, &c, &h, &w,
        &nStride, &cStride, &hStride, &wStride));
...
void SoftmaxCrossEntropy::forward_pass()
{
    ...
    // Do softmax forward pass using accurate softmax and
